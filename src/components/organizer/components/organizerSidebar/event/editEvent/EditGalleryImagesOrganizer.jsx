...
const EditGalleryImagesOrganizer = ({
  eventGalleryImages,
  ...
  // console.log({eventGalleryImages})
      ...
      if (file.size < maxSize) {
        ...
        img.onload = () => {
          ...
          if (img.width !== 612 || img.height !== 408) {
            toast.error("Image dimensions must be 600x281.");
          } else {
  ...
  const handleAddClick = async () => {
    ...
    try {
      ...
      formData.append("GalleryImage", selectedImage);

      let response = await axios.post(
        `${eventEndPoint.UPDATE_EVENT_GALLERY_IMAGE}`,
        formData
      );

      toast.success("Event Gallery  Image uploaded successfully");
      // console.log(response.data);
      setEventGalleryImages(response.data.data);
      setSelectedImage();
      setImageUrl("");
    } catch (error) {
      if (error.response) {
  ...
  const handlerRemoveImage = async (imageData) => {
    try {
      ...
      let response = await axios.post(
        `${eventEndPoint.DELETE_EVENT_GALLERY_IMAGE}`,
        payload
      );

      toast.success(response.data.message);

      const fillteredProducts = eventGalleryImages.filter(
        (eventGalleryImages) => eventGalleryImages._id !== imageData._id
      );
      setEventGalleryImages(fillteredProducts);
    } catch (error) {
      if (error.response) {
        const { status, data } = error.response;
...
export default EditGalleryImagesOrganizer;

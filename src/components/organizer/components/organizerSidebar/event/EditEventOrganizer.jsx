...
import useFetchEventData from "../../../../superAdmin/hooks/useFetchEventData";
import { useFormik } from "formik";
import EditGalleryImages from "../../../../superAdmin/components/sidebar/event/EditEvent/EditGalleryImages";
import EditFAQModal from "../../../../superAdmin/components/sidebar/event/EditEvent/EditFAQModal";
import EditAddnewFAQ from "../../../../superAdmin/components/sidebar/event/EditEvent/EditAddnewFAQ";
import EditEventTimeDateModal from "../../../../superAdmin/components/sidebar/event/EditEvent/EditEventTimeDateModal";
import { formatDate3 } from "../../../../common/formatDate2";
import EditAddress from "../../../../common/EditAddress";
import EditCarouselImage from "../../../../superAdmin/components/sidebar/event/EditEvent/EditCarouselImage";
...
import EditCarouselImageOrganizer from "./editEvent/EditCarouselImageOrganizer";
import EditGalleryImagesOrganizer from "./editEvent/EditGalleryImagesOrganizer";
import { MdOutlineInfo } from "react-icons/md";

const EditEventOrganizer = () => {
  const [disableToggleButton, setDisableToggleButton] = useState(true);
  const { _id } = useParams();
  ...
  useEffect(() => {
    if (allDataFetched) {
      getAllOrganizerEventData();
    }
  }, [allDataFetched]);

  const getAllOrganizerEventData = async () => {
    try {
      const payload = {
        event_id: _id,
      };
      ...
      // Check if we want to show save as draft button and publish button
      // if (
      //   (response.data.data.EventStatus != "1" ||
      //     response.data.data.EventStatus !== "4" ||
      //     response.data.data.EventStatus == "2") &&
      //   response.data.data.CreatedBy === "3"
      // ) {
  ...
  const {
    ...
    onSubmit: async (values) => {
      ...
      const category = [
        {
          category_id: categoryID[0].value,
        },
      ];
      ...
      try {
        let response = await axios.post(
          `${organizerEndpoint.UPDATE_ORGANIZER_DATA_URL}`,
          formData
        );

        toast.success(response.data.message);
        setTimeout(() => {
          navigate("/organizer/dashboard/event");
        }, 2000);
  ...
  return (
    <div>
      ...
      <div className="mt-[3%] ml-[2%]">
        <OrganizerBreadCrumb path={"Event"} />
        <div className="w-full flex justify-end">
          <button
            type="button"
            onClick={() => navigate("/organizer/dashboard/event")}
            className={`w-[15%] py-2 
      ...
      {!loading ? (
        <>
          {eventData.EventStatus === 4 && (
            <div
              className="flex w-full pl-4 items-center mt-4 p-4 mb-4 text-sm text-yellow-800 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 dark:border-yellow-800"
              role="alert"
            >
              <svg
              ...
              <div>
                <span className="font-medium pr-2">Rejection Remark!</span>
                {eventData.EventRejectRemark}
              </div>
            </div>
          )}
                            ...
                            <div className="absolute  top-1 left-10 ml-2 flex flex-col items-center group">
                              ...
                              <div className="absolute bottom-0  flex-col items-center hidden mb-5 group-hover:flex">
                                <span className="relative w-96 left-32 rounded-md z-10 p-4 text-xs leading-relaxed text-white whitespace-no-wrap bg-black shadow-lg">
                                  ...
                                  <p className="mt-2">
                                    Venue Not Listed: If your venue is not
                                    listed, select “Not found, contact admin.”
                                  </p>
                                </span>
                                <div className="w-3 h-3 -mt-2 rotate-45 bg-black"></div>
                              </div>
                      ...
                      {selectedLocationType === "tba" && (
                        <div className=" md:col-span-2  mt-4 w-full">
                          <EditAddress
                            ...
                            CityIsoCode={
                              eventData?.VenueToBeAnnouncedCityIsoCode
                            }
                            // State value
                          ...
                          {(eventData.EventStatus == 1 ||
                            ...
                            <span>
                              <TbEdit
                                ...
                                onClick={() =>
                                  handlerEditEventDateandTime(event)
                                }
                              />
                            </span>
                          )}

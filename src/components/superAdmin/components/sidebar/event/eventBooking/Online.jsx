...
const Online = ({
  ...
  const [bookingsOpenModal, setBookingsOpenModal] = useState(false);
  const [selectedBookingId, setSelectedBookingId] = useState(null);
  const [selectedAction, setSelectedAction] = useState(null);

  const handleAction = (ticketId, action) => {
    console.log(action);
    setSelectedBookingId(ticketId);
    setSelectedAction(action);
    setBookingsOpenModal(true);
    console.log(action);
  };
  ...
  const getBookingData = async () => {
    ...
    try {
      ...
      setTotalQuantity(fetchBooking.data.data.totalQuantity);
      setTotalCheckin(fetchBooking.data.data.totalCheckInCount);
      setTotalPrice(fetchBooking.data.data.totalPrice);
      setLoading(false);
    } catch (error) {
      console.log(error);
      if (error.response) {
  ...
  const DownloadReportHandler = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", "online_Report.xlsx");
  ...
  return (
    <>
      <div className="mt-4 md:ml-6">
        <Toaster />
        <div className="flex md:flex-row  gap-2 flex-col md:gap-10 mb-10 ">
          {selectedBookingType === 2 && (
            <div className="flex  md:items-end mt-8 gap-6 md:gap-x-11  mx-auto ">
              {tabs.map((event) => (
                <div key={event.id}>
                  <button
          ...
          <div className="flex md:flex-row  gap-2 flex-col md:gap-4 w-[100%] ">
            ...
            <div className="h-32  w-full bg-gray-100 flex flex-col justify-center px-4 ">
              <p className="capitalize text-xl ">total checkin</p>
              <p className="capitalize text-xl font-semibold">{totalCheckin}</p>
            </div>
            <div className="h-32  w-full bg-gray-100 flex flex-col justify-center px-4 ">
              <p className="capitalize text-xl ">total price</p>
              <p className="capitalize text-xl font-semibold">
        ...
        <div className="w-[94%] ">
          <div className="w-[100%]  flex flex-col gap-4 justify-between">
            ...
            <div className="flex   md:mt-0 mt-4 gap-x-4">
              ...
              <div className="relative inline-block text-left">
                ...
                {isOpen && (
                  <div
                    id="dropdown"
                    className="z-10 absolute -right-16 mt-2 w-44  bg-white  divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700"
                  >
                    <ul
          ...
          <div className="fixed top-0 left-0 w-full h-[100vh] bg-black bg-opacity-30 flex items-center justify-center z-50 overflow-y-auto">
            <div className="bg-white p-4 rounded-md md:w-[60%] lg:w-[50%] sm:w-[70%]">
              ...
              <div className="">
                ...
                <div className="flex w-full mt-4 gap-x-5 justify-center">
                  <div className="w-[50%]">
      ...
      <div className="w-[100%] md:mt-0 mt-6 mx-auto border-b-2 pb-4 border-Gray85">
                ...
                {allBookingData?.length >= 1 ? (
                  allBookingData?.map((eventData, index) => (
                    ...
                    >
                      ...
                      <td className="pl-4 py-4">Online</td>
                      ...
                      <td className="pl-6 py-4">{eventData.CheckIn}</td>
                      <td className="pl-2 py-4">{eventData.TicketName}</td>
                      <td className="pl-10 py-4">
                        {eventData.EventTicketType}
                      </td>
                      <td className="pl-8 py-4">{eventData.Transaction_id}</td>
                      <td className="pl-6 py-4">{eventData.TicketPrice}</td>
                      <td className="pl-8 py-4">
                        {eventData.PromocodeName
                          ? eventData.PromocodeName
                          : "-"}
                      </td>
                      <td className="pl-4 py-4">
                        {eventData.PromocodeValue
                          ? eventData.PromocodeValue
                          : "-"}
                      </td>
                      ...
                      {eventData.status === bookingStatus.Booked ? (
                        <td className="pr-14 pl-6 py-4">
                          <div className="flex gap-2">
                            <span
                              className="cursor-pointer"
                              onClick={() =>
                                handleAction(eventData.Booking_id, "cancel")
                              }
                            >
                              Cancel
                            </span>
                            <span
                              ...
                              onClick={() =>
                                handleAction(eventData.Booking_id, "resend")
                              }
                            >
                              Resend
                            </span>
            ...
            <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
              <div className="bg-white p-6 rounded-lg shadow-lg">
                ...
                <p>
                  ...
                  {selectedAction === "resend" ? "Resend" : "Cancel"} booking?
                </p>

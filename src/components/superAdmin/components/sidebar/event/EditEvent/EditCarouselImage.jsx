...
const EditCarouselImage = ({ eventCarouselImages, setEventCarouselImages }) => {
  const [imageUrl, setImageUrl] = useState();
  const [selectedImage, setSelectedImage] = useState(null);

  // console.log({eventCarouselImages})
  ...
  const handlerRemoveImage = async (imageData) => {
    try {
      ...
      const fillteredProducts = eventCarouselImages.filter(
        (eventCarouselImages) => eventCarouselImages._id !== imageData._id
      );
      setEventCarouselImages(fillteredProducts);
    } catch (error) {
      if (error.response) {
        const { status, data } = error.response;
